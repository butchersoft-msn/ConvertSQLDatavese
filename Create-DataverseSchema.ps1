...
foreach ($EntityMap in $EntityMaps)
{
    ...
    foreach($Field in $Fields)
    {
        ...
        switch ($dataType) {
            ...
            "sql_variant"        { $crmAttributeType =  "String" }  # No direct equivalent, default to String
            ...
            "hierarchyid"        { $crmAttributeType =  "String" }  # No direct equivalent, default to String
            "geometry"           { $crmAttributeType =  "String" }  # No direct equivalent, default to String
            "geography"          { $crmAttributeType =  "String" }  # No direct equivalent, default to String
            ...
            "timestamp"          { $crmAttributeType =  "DateTime" }  # No direct equivalent, assuming timestamp as DateTime
            "nvarchar"           { $crmAttributeType =  "String"; $maxLength = if ($null -eq $maxLength ) { 255 } else { [int]$maxLength } }
            "nchar"              { $crmAttributeType =  "String"; $maxLength = if ($null -eq $maxLength ) { 255 } else { [int]$maxLength } }
            "xml"                { $crmAttributeType =  "Memo" }
            "sysname"            { $crmAttributeType =  "String" }
            "urilisttype"        { $crmAttributeType =  "String" }  # No direct equivalent, default to String
            default              { $crmAttributeType =  "String"; $maxLength = if ($null -eq $maxLength ) { 255 } else { [int]$maxLength } } 
        }


        #Set the first string field to primary attribute
        if($PrimaryNameAttribute -eq $null -and $dataType -eq "String") 
        { 
            $PrimaryNameAttribute = $Field
        }

        if($Field.column_name -eq $Fields[1].column_name)
        {
            $SecondField = $Field
        }
    ...
    if($PrimaryNameAttribute -eq $null) 
    { 

        $PrimaryNameAttribute = $SecondField
    }
    
    $EntityMap.PrimaryNameAttribute += $FieldMetadata
}
