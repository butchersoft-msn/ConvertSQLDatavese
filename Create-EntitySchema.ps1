...
foreach ($EntityMap in $EntityMaps)
{
    ...
    foreach($Field in $Fields)
    {
        ...
        if($PrimaryNameAttribute -eq $null -and $dataType -eq "String") { $PrimaryNameAttribute = $Field.column_name }

        #Map Table Field to Entitiy Field
        $FieldMetadata = @{
            AttributeSchemaName = $Field.column_name.ToLower()
    ...
    if($PrimaryNameAttribute -eq $null) { $PrimaryNameAttribute = $Fields[1].column_name }
    $EntityMap.PrimaryNameAttribute = $PrimaryNameAttribute
}
